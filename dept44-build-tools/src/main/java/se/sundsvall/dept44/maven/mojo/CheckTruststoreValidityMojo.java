...
public class CheckTruststoreValidityMojo extends AbstractDept44CheckMojo {
	...
	private static final String FAILURE_MESSAGE =
			"Certificate '%s' expiration date (%s) is before or less than %d months from now (%s) and needs to be updated";
			...
			if (certificateFiles != null) {
				for (var certificateFile : certificateFiles) {
					try (var certificateInputStream = new FileInputStream(certificateFile)) {
						...
						var notAfter = certificate
								.getNotAfter()
								.toInstant()
								.atZone(ZoneId.systemDefault())
								.toLocalDate();
						if (notAfter.isBefore(expiry)) {
							addError(FAILURE_MESSAGE.formatted(
									certificateFile.getName(),
									notAfter.format(ISO_DATE),
