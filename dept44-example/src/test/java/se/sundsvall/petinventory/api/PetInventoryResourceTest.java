...
class PetInventoryResourceTest {
	...
	void postPetImage() {
		...
		when(petInventoryServiceMock.savePetImage(anyLong(), any(MultipartFile.class)))
				.thenReturn(imageId);

		// Act
		webTestClient
				.post()
				.uri("/pet-inventory-items/{id}/images", id)
				...
				.contentType(ALL)
				.expectHeader()
				.location("/pet-inventory-items/" + id + "/images/" + imageId);
	...
	void getPetImage() throws IOException {
		...
		final var response = webTestClient
				.get()
				.uri("/pet-inventory-items/{id}/images/{petImageId}", id, petImageId)
				.exchange()
				.expectStatus()
				.isOk()
				.expectHeader()
