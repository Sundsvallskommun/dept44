...
public class ValidSortByPropertyConstraintValidator
		implements ConstraintValidator<ValidSortByProperty, AbstractParameterPagingAndSortingBase> {

	private static final String CUSTOM_ERROR_MESSAGE_TEMPLATE =
			"One or more of the sortBy properties %s are not valid. Valid properties to sort by are %s.";
	...
	public boolean isValid(
			final AbstractParameterPagingAndSortingBase parameters, final ConstraintValidatorContext context) {
	...
	private void useCustomMessageForValidation(
			final ConstraintValidatorContext constraintContext, final List<String> sortBy) {
		constraintContext.disableDefaultConstraintViolation();
		constraintContext
				.buildConstraintViolationWithTemplate(InterpolationHelper.escapeMessageParameter(
						String.format(CUSTOM_ERROR_MESSAGE_TEMPLATE, sortBy, entityProperties)))
