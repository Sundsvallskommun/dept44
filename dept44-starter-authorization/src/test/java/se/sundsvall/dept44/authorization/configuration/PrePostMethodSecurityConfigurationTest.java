...
class PrePostMethodSecurityConfigurationTest {
	...
	void verifyConfigurationAnnotation() {
		assertThat(getAnnotation(PrePostMethodSecurityConfiguration.class, Configuration.class))
				.isNotNull();
	}

	@Test
	void verifyEnableGlobalMethodSecurityAnnotation() {
		final EnableMethodSecurity annotation =
				getAnnotation(PrePostMethodSecurityConfiguration.class, EnableMethodSecurity.class);
	...
	void verifyBeanCreation(
		...
		assertThat(configuration.jwtAuthorizationExtractionFilter(
						properties, jwtTokenUtil, webAuthenticationDetailsSource, applicationContext, objectMapper))
				.isNotNull()
				.isInstanceOf(JwtAuthorizationExtractionFilter.class);
		assertThat(configuration.webAuthenticationDetailsSource())
				.isNotNull()
				.isInstanceOf(WebAuthenticationDetailsSource.class);
		assertThat(configuration.jwtTokenUtil(jwtAuthorizationProperties))
				.isNotNull()
				.hasFieldOrPropertyWithValue("secret", secret.getBytes());
	}
}
