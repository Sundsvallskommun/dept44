...
class JwtTokenUtilTest {
	...
	private final String expiredMessageRegexMatch =
			"^JWT expired (\\d+) milliseconds ago at 1971-01-01T00:00:00.000Z. Current time: (\\d{4}-\\d{2}-\\d{1,2}T\\d{1,2}:\\d{1,2}:\\d{1,2}.\\d{0,3})Z. Allowed clock skew: 0 milliseconds.$";
	...
	void verifyComponentAnnotation() {
		assertThat(AnnotationUtils.getAnnotation(JwtTokenUtil.class, Component.class))
				.isNotNull();
	}
	...
	void constructorWithNull() {
		final IllegalArgumentException exception =
				assertThrows(IllegalArgumentException.class, () -> new JwtTokenUtil(null));
	...
	void processExpiredJwt(@Load("expired_jwt.txt") String jwt) {
		...
		final ExpiredJwtException exception =
				assertThrows(ExpiredJwtException.class, () -> util.getUsernameFromToken(jwt));
	...
	void processInvalidSignatureJwt(@Load("invalid_signature_jwt.txt") String jwt) {
		...
		final SignatureException exception =
				assertThrows(SignatureException.class, () -> util.getUsernameFromToken(jwt));
	...
	void processMalformedJwt(@Load("malformed_jwt.txt") String jwt) {
		...
		final MalformedJwtException exception =
				assertThrows(MalformedJwtException.class, () -> util.getUsernameFromToken(jwt));
