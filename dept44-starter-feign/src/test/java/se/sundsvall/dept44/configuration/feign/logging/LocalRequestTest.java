...
class LocalRequestTest {
	...
	void schemeDoesntExist() {
		...
		assertThat(unit.getScheme()).isEmpty();
	}
	...
	void hostDoesntExist() {
		...
		assertThat(unit.getHost()).isEmpty();
	}
	...
	void pathDoesntExist() {
		final Request request = Request.create(Request.HttpMethod.GET, "mailto:invalidpath@mail.com",
			Collections.emptyMap(), Request.Body.create(new byte[0]), new RequestTemplate());

		assertThat(unit(request).getPath()).isEmpty();
	}
	...
	void contentTypeExists() {
		...
		Request request = Request.create(Request.HttpMethod.GET, "https://localhost:8080", headers,
			Request.Body.create(new byte[0]), new RequestTemplate());
	...
	@Test
	void parseCharset() {
		Request request = Request.create(Request.HttpMethod.GET, "https://localhost:8080", Collections.emptyMap(),
			Request.Body.create("test", StandardCharsets.UTF_16BE), new RequestTemplate());

		assertThat(unit(request).getCharset()).isEqualTo(StandardCharsets.UTF_16BE);
	}

	private Request get(String uri) {
		return Request.create(Request.HttpMethod.GET, uri, Collections.emptyMap(), Request.Body.create(new byte[0]),
			new RequestTemplate());
	}

	private Request getEmpty() {
		return Request.create(Request.HttpMethod.GET, "", Collections.emptyMap(), Request.Body.create(new byte[0]),
			new RequestTemplate());
	}
