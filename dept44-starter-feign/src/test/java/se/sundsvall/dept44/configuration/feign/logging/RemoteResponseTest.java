...
class RemoteResponseTest {
	...
	private Response request() {
		return Response.builder().status(200).body("hello world", StandardCharsets.UTF_8)
			.request(Request.create(Request.HttpMethod.GET, "https://localhost:8080", Collections.emptyMap(),
	...
	private Response requestWithHeaders() {
		...
		return Response.builder().status(200).body("hello world", StandardCharsets.UTF_8)
			.request(Request.create(Request.HttpMethod.GET, "https://localhost:8080", headers,
				Request.Body.create(new byte[0]), new RequestTemplate()))
	...
	void nullContentType() throws IOException {
		assertThat(unit(request()).getContentType()).isNull();
	}
