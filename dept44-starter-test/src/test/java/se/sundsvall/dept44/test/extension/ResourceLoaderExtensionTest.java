...
class ResourceLoaderExtensionTest {
	...
	@Test
	void deserializeJson(@Load(value = TEST_JSON_FILE, as = Load.ResourceType.JSON) TestObject testObject) {
	...
	@Test
	void deserializeXml(@Load(value = TEST_XML_FILE, as = Load.ResourceType.XML) TestObject testObject) {
		assertThat(testObject).isNotNull().hasNoNullFieldsOrProperties();
		assertThat(testObject.getKey()).isEqualTo("this-is-a-key");
		assertThat(testObject.getValue()).isEqualTo("this-is-a-value");
		assertThat(testObject.getOffsetDateTime()).isAtSameInstantAs(OffsetDateTime.parse("2022-01-01T00:00:00+02:00", DateTimeFormatter.ISO_OFFSET_DATE_TIME));
	...
	@Test
	void deserializeString(@Load(TEST_JSON_FILE) String testObject) {
	...
	/**
	 * @see CustomTestExecutionExceptionHandler for the tests below.
	 *      This class is necessary in these tests due to the testing of functionality
	 *      that happens before the actual test implementation. We must "swallow" the exceptions generated during the
	...
	class AnnotationExceptionTest {
		...
		@Test
		void deserializeJsonWhenFileDoesntMatchObject(@Load(value = TEST_XML_FILE, as = Load.ResourceType.JSON) TestObject testObject) {
			fail("Should have failed when the method parameters was resolved!");
		}

		@Test
		void deserializeXmlWhenFileDoesntMatchObject(@Load(value = TEST_JSON_FILE, as = Load.ResourceType.XML) TestObject testObject) {
			fail("Should have failed when the method parameters was resolved!");
		}

		@Test
		void deserializeWhenFileDoesntExist(@Load("not-existing.xml") TestObject testObject) {
			fail("Should have failed when the method parameters was resolved!");
		}
	}
}
