...
class ResourceLoaderExtensionTest {
	...
	void deserializeJson(@Load(value = TEST_JSON_FILE, as = Load.ResourceType.JSON) final TestObject testObject) {
		...
		assertThat(testObject.getOffsetDateTime())
				.isAtSameInstantAs(
						OffsetDateTime.parse("2022-01-01T00:00:00+02:00", DateTimeFormatter.ISO_OFFSET_DATE_TIME));
	}
	...
	class AnnotationExceptionTest {
		...
		@Test
		void deserializeJsonWhenFileDoesntMatchObject(
				@Load(value = TEST_XML_FILE, as = Load.ResourceType.JSON) final TestObject testObject) {
			fail("Should have failed when the method parameters was resolved!");
		}

		@Test
		void deserializeXmlWhenFileDoesntMatchObject(
				@Load(value = TEST_JSON_FILE, as = Load.ResourceType.XML) final TestObject testObject) {
			fail("Should have failed when the method parameters was resolved!");
		}
