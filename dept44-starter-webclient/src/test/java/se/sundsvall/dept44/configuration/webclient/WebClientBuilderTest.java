...
class WebClientBuilderTest {
	...
	void testValueRestrictions() {
		...
		assertThat(assertThrows(IllegalArgumentException.class, () -> builder.withBaseUrl(null)))
				.hasMessage("baseUrl cannot be null or blank");
		assertThat(assertThrows(IllegalArgumentException.class, () -> builder.withBaseUrl("")))
				.hasMessage("baseUrl cannot be null or blank");
		assertThat(assertThrows(IllegalArgumentException.class, () -> builder.withBaseUrl(" ")))
				.hasMessage("baseUrl cannot be null or blank");
		assertThat(assertThrows(IllegalArgumentException.class, () -> builder.withConnectTimeout(null)))
				.hasMessage("connectTimeout may not be null.");
		assertThat(assertThrows(IllegalArgumentException.class, () -> builder.withReadTimeout(null)))
				.hasMessage("readTimeout may not be null.");
		assertThat(assertThrows(IllegalArgumentException.class, () -> builder.withWriteTimeout(null)))
				.hasMessage("writeTimeout may not be null.");
	}
	...
	void testBuildFromCustomValuesWithBasicAuth() {
		...
		assertThat(webClient)
				...
				.asString()
				.isEqualTo("[Authorization:\"Basic dXNlck5hbWU6cGFzc3dvcmQ=\"]");
		assertThat(webClient)
				.extracting("builder")
				.extracting("baseUrl")
				.asString()
	...
	void testBuildWithStatusHandler() {
		...
		webClient = createBuilder(false)
				.withStatusHandler(
						...
						clientResponse -> Mono.just(Problem.valueOf(Status.INTERNAL_SERVER_ERROR)))
				.build();
	...
	void testBuildWithServiceType() {
		...
		try (var mockStaticWebClientAdapter = mockStatic(WebClientAdapter.class);
			...
			mockStaticWebClientAdapter
					.when(() -> WebClientAdapter.create(any(WebClient.class)))
					.thenReturn(mockWebClientAdapter);
			mockStaticHttpServiceProxyFactory
					.when(() -> HttpServiceProxyFactory.builderFor(any(HttpExchangeAdapter.class)))
	...
	private void assertTimeoutSetting(WebClient webClient, int timeoutMillis) {
		assertThat(webClient)
				...
				.containsEntry(ChannelOption.CONNECT_TIMEOUT_MILLIS, timeoutMillis);
	}
